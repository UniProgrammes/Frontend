...
interface StudyPlanCardProps {
  ...
  return (
    <div className="bg-white shadow-md rounded-3xl p-4 w-fit">
      ...
      <p className="m-2 mt-8">{completed ? "Completed" : "Not Completed"}</p>
      <p className="m-2"> {numCourses} courses selected</p>
      <p className="m-2 mb-8">Last Updated on: {lastUpdate}</p>
      <button className="px-4 py-2 bg-purple-500 text-white rounded-full text-center w-full">Modify Study Plan</button>
    </div>
  )
}
